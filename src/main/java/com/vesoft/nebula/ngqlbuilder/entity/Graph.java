...
import com.vesoft.nebula.ngqlbuilder.query.util.Util;
...
public class Graph {
    ...
    private static final String BY_TAG = "By Tag";
    private static final String BY_EDGE = "By Edge";
    ...
    /**
     * get indexList by tagName,if you pass in null,return all index.
     *
     * @param tagName tagName
     * @return indexList
     */
    public List<String> getTagIndexes(String tagName) throws UnsupportedEncodingException {
        StringBuilder tagIndexJoin = new StringBuilder();
        tagIndexJoin.append(KeyWord.SHOW).append(" ").append(KeyWord.TAG).append(" ")
            .append(KeyWord.INDEXES);
        ResultSet showTagIndexes = run(tagIndexJoin.toString());
        List<ValueWrapper> indexesName = showTagIndexes.colValues(INDEX_NAME);
        ArrayList<String> allIndexesName = new ArrayList<>();
        for (ValueWrapper index : indexesName) {
            allIndexesName.add(index.asString());
        }
        if (tagName == null) {
            return allIndexesName;
        } else {
            List<ValueWrapper> byTags = showTagIndexes.colValues(BY_TAG);
            ArrayList<String> allTags = new ArrayList<>();
            for (ValueWrapper index : byTags) {
                allTags.add(index.asString());
            }
            HashMap<String, List<String>> tagIndexesMap = new HashMap<>();
            for (int i = 0; i < allTags.size(); i++) {
                String tag = allTags.get(i);
                if (tagIndexesMap.get(tag) == null) {
                    ArrayList<String> indexes = new ArrayList<>();
                    indexes.add(allIndexesName.get(i));
                    tagIndexesMap.put(tag,indexes);
                } else {
                    tagIndexesMap.get(tag).add(allIndexesName.get(i));
                }
            }
            return tagIndexesMap.get(tagName);
        }
    }

    /**
     * get indexList by edgeName,if you pass in null,return all index.
     *
     * @param edgeName edgeName
     * @return indexList
     */
    public List<String> getEdgeIndexes(String edgeName) throws UnsupportedEncodingException {
        StringBuilder edgeIndexJoin = new StringBuilder();
        edgeIndexJoin.append(KeyWord.SHOW).append(" ").append(KeyWord.EDGE).append(" ")
            .append(KeyWord.INDEXES);
        ResultSet showTagIndexes = run(edgeIndexJoin.toString());
        List<ValueWrapper> indexesName = showTagIndexes.colValues(INDEX_NAME);
        ArrayList<String> allIndexesName = new ArrayList<>();
        for (ValueWrapper index : indexesName) {
            allIndexesName.add(index.asString());
        ...
        if (edgeName == null) {
            ...
            List<ValueWrapper> byTags = showTagIndexes.colValues(BY_EDGE);
            ArrayList<String> allTags = new ArrayList<>();
            for (ValueWrapper index : byTags) {
                allTags.add(index.asString());
            }
            ...
            return tagIndexesMap.get(edgeName);
        }
    }
