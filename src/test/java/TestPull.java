...
public class TestPull extends TestDataBase {
    ...
    public void testPullRelationship() throws UnsupportedEncodingException {
        ...
        Relationship relationship = new Relationship(1, 2, "subject", edgeValue, 1);
    ...
    public void testPullSubgraph() throws UnsupportedEncodingException {
        ...
        graph.run("upsert edge on subject 1->2@1 set teacherName = \"XuXin\"");
        graph.run("upsert vertex on person 3 set name = \"wm\"");
        graph.pull(subgraph);
        assert subgraph.getVertexes().get(2).getPropMap().get("person")
